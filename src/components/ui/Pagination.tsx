...
export default function Pagination({
  ...
  const goToPage = (p: number) => onPageChange(Math.min(totalPages, Math.max(1, p)));
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <nav className="inline-flex items-center gap-1.5" aria-label="Pagination">
          <IconButton label="First page" disabled={!canPrev} onClick={() => goToPage(1)} icon={ChevronDoubleLeftIcon} />
          <IconButton label="Previous page" disabled={!canPrev} onClick={() => goToPage(page - 1)} icon={ChevronLeftIcon} />

          <span className="mx-2 min-w-[6rem] text-center text-sm text-zinc-700">
            Page <span className="font-medium">{page}</span> / {totalPages}
          </span>

          <IconButton label="Next page" disabled={!canNext} onClick={() => goToPage(page + 1)} icon={ChevronRightIcon} />
          <IconButton label="Last page" disabled={!canNext} onClick={() => goToPage(totalPages)} icon={ChevronDoubleRightIcon} />
        </nav>
      </div>
    </div>
  );
...
function ChevronDoubleRightIcon(props: Readonly<React.SVGProps<SVGSVGElement>>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} {...props}>
