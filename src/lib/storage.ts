// src/lib/storage.ts
import type { LeadsViewState, LeadStatus, SortDir, SortKey } from '@/types/types';
...
function isValidLeadsViewState(candidate: unknown): candidate is LeadsViewState {
...
export function saveLeadsViewState(state: LeadsViewState): void {
  const sanitizedState: LeadsViewState = {
    ...
    statusFilter: isValidStatus(state.statusFilter) ? state.statusFilter : 'all',
